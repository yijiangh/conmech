...
def repetitive_test_stiffness_checker(frame_file_path, engine, parsed_pt_loads=None, include_sw=False,
    ...
    print('sol benchmark: success? {6} | max_trans {0} / tol {4}, max_rot {1} / tol {5}, max_trans_id {2}, max_rot_id {3}'.format(\
        max_trans, max_rot, max_t_id, max_r_id, trans_tol, rot_tol, sol_success))

    if expect_partial_ids_success:
        assert sc.get_compliance() > 0.0
    ...
    for _ in range(n_attempts):
        ...
        for i in eR.keys(): 
            ...
            assert_almost_equal(eR[i][1], tmp_eR[i][1]) 
    tot_time = time.time() - st_time
    print('with reinit: avg time: {} s, total_time: {} s for {} runs'.format(tot_time / n_attempts, tot_time, n_attempts))
    cprint('Pass.', 'green')
...
def test_stiffness_checker_solve_consistency(test_case, load_case, test_data_dir, n_attempts, engine):
    if test_case == 'tower':
        ...
        load_file_name = 'tower_3D_loads.json'
        ...
        load_file_name = 'topopt-100_loads.json'

        # a big cantilever are floating in this partial ids
        failed_existing_ids = [1, 56, 59, 60, 66, 67, 68, 69, 83, 84, 102, 130]
...
def test_get_nodal_loads(engine):
    ...
    for pt_load in parsed_pt_loads:
        ...
        assert_equal(pt_load.force + pt_load.moment, fetched_nodal_loads[node_id])
...
def test_neighnor_query(engine):
    ...
    assert_equal(sc.get_element_connected_node_ids(fix_node_only=True), [0,2,4,6]) 
    assert_equal(sc.get_element_connected_node_ids(existing_ids=[0,3], fix_node_only=True), [0,6]) 
    assert_equal(set(sc.get_element_connected_node_ids(existing_ids=[0,3], fix_node_only=False)), set([0,6,1,7])) 
...
def repetitive_test_equilibrium(frame_file_path, engine, parsed_pt_loads=None, include_sw=False,
    ...
    if debug and sol_success:
        ...
        # scc.set_loads(point_loads=parsed_pt_loads, gravity_load=gravity_load)
...
def test_nodal_equilibrium(test_case, load_case, engine, debug):
    ...
    if test_case == 'tower':
        ...
        # some parts are floating in this partial ids
        failed_existing_ids = [0, 5, 6, 12, 15, 16]        
        if load_case == 'self_weight' or load_case == 'self_weight+point_load':
            trans_tol = 0.23
...
def test_uniformly_distributed_load_with_gravity(test_case, existing_e_ids, engine):
    ...
    # checking explicit, manually constructed gravity load consistent with the result from setting gravity_load

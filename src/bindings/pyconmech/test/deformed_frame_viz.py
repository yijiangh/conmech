...
def meshcat_visualize_deformed(meshcat_vis, beam_disp, orig_shape=None, disc=10, scale=1.0, time_step=1):
    red = np.array([1, 0, 0])
    blue = np.array([0, 0, 1])
...
#         print("color {0}".format(color))
        ...
        meshcat_vis[or_key].set_object(g.Line(g.PointsGeometry(orig_beam_pts.T), g.MeshBasicMaterial(rgb_to_hex(black),opacity=0.6)))
